...
import bearsApi from '../api';
import bearsAuth from '../auth';
import { camelCase } from '../utils';

const debug = newDebug('bears:broadcast');
const noop = function() {}
const formatter = formatterFactory(bearsApi);

const bearsBroadcast = {};

// Base transaction logic -----------------------------------------------------

/**
 * Sign and broadcast transactions on the bears network
 */

bearsBroadcast.send = function bearsBroadcast$send(tx, privKeys, callback) {
  const resultP = bearsBroadcast._prepareTransaction(tx)
    .then((transaction) => {
      debug(
      ...
      return Promise.join(
        transaction,
        bearsAuth.signTransaction(transaction, privKeys)
      );
    })
    .spread((transaction, signedTransaction) => {
      ...
      return bearsApi.broadcastTransactionSynchronousAsync(
        signedTransaction
      ).then((result) => {
...
bearsBroadcast._prepareTransaction = function bearsBroadcast$_prepareTransaction(tx) {
  const propertiesP = bearsApi.getDynamicGlobalPropertiesAsync();
  return propertiesP
    .then((properties) => {
      // Set defaults on the transaction
      const chainDate = new Date(properties.time + 'Z');
      const refBlockNum = (properties.last_irreversible_block_num - 1) & 0xFFFF;
      return bearsApi.getBlockAsync(properties.last_irreversible_block_num).then((block) => {
...
operations.forEach((operation) => {
  ...
  bearsBroadcast[`${operationName}With`] =
    function bearsBroadcast$specializedSendWith(wif, options, callback) {
      debug(`Sending operation "${operationName}" with`, {options, callback});
      const keys = {};
      if (operation.roles && operation.roles.length) {
        keys[operation.roles[0]] = wif; // TODO - Automatically pick a role? Send all?
      }
      return bearsBroadcast.send({
        extensions: [],
  ...
  bearsBroadcast[operationName] =
    function bearsBroadcast$specializedSend(wif, ...args) {
      ...
      return bearsBroadcast[`${operationName}With`](wif, options, callback);
    };
});

const toString = obj => typeof obj === 'object' ? JSON.stringify(obj) : obj;
broadcastHelpers(bearsBroadcast);

Promise.promisifyAll(bearsBroadcast);

exports = module.exports = bearsBroadcast;
